...
import { ArrowLeft, ArrowRight, LayoutDashboard, Zap } from 'lucide-react';
...
export default function AdminStaticsPage() {
  ...
  useEffect(() => {
    ...
    if (authRole && authRole !== 'admin') {
      ...
      router.push('/login?redirect=/admin/statics');
    } else {
    ...
    <div className="min-h-screen bg-[#0A0A0E] text-[#F5F5F7]">
      ...
      <main className="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8 space-y-6">
        ...
        <div className="flex items-center justify-between">
          <Link
            href="/admin"
            className="inline-flex items-center gap-2 px-3.5 py-2 rounded-xl bg-white/5 hover:bg-white/10 text-xs font-semibold text-[#A1A1AA] hover:text-white border border-white/10 transition-colors"
          >
        ...
        {/* The Statistics & Analytics Suite */}
        <AdminStatistics />
      </main>
    </div>
  );
