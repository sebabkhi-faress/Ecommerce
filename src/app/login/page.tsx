...
function LoginForm() {
  ...
  return (
    <div className="min-h-[88vh] flex items-center justify-center px-4 py-10 bg-[var(--obsidian)] text-[var(--white-titanium)] relative overflow-hidden select-none transition-colors duration-200">
      ...
      <div className="w-full max-w-md relative z-10 space-y-4">
        {/* Top Bar: Back to Store + Language Switcher (Single theme toggle stays in Navbar only) */}
        <div className="flex items-center justify-between px-1">
          <Link
            href="/"
            className="inline-flex items-center gap-1.5 text-xs font-semibold text-[var(--silver-muted)] hover:text-[var(--white-titanium)] transition-colors group"
          >
            {lang === 'ar' ? (
          ...
          <button
            ...
            className="px-2.5 py-1 rounded-xl bg-[var(--smoke)] text-[var(--silver-muted)] hover:text-[var(--white-titanium)] border border-[var(--border-subtle)] transition-colors text-xs font-mono font-bold cursor-pointer"
        ...
        <div
          className={`relative rounded-3xl p-[1px] bg-gradient-to-b from-[#FF6B00]/40 via-[var(--border-subtle)] to-transparent shadow-xl transition-transform ${
            shake ? 'animate-shake' : ''
          }`}
        >
          <div className="bg-[var(--charcoal)] rounded-[23px] p-6 sm:p-8 space-y-6 backdrop-blur-2xl border border-[var(--border-subtle)] transition-colors duration-200">
            ...
            <div className="text-center space-y-2">
              ...
              <div className="space-y-1">
                <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-[#FF6B00]/10 border border-[#FF6B00]/25 text-[#FF6B00] text-[10px] font-mono font-bold uppercase tracking-wider mb-1">
                  <ShieldCheck className="w-3 h-3" />
                  <span>{lang === 'ar' ? 'بوابة الدخول الآمنة' : 'ESPACE SÉCURISÉ'}</span>
                </div>
                <h1 className="text-xl sm:text-2xl font-black text-[var(--white-titanium)] tracking-tight">
                  {lang === 'ar' ? 'تسجيل الدخول' : 'Connexion au Compte'}
                </h1>
                <p className="text-xs text-[var(--silver-muted)] leading-relaxed max-w-xs mx-auto">
                  {lang === 'ar'
            ...
            {/* Error Message Box */}
            {error && (
              <div className="p-3.5 bg-red-500/15 border border-red-500/30 text-red-400 dark:text-red-300 rounded-2xl text-xs flex items-start gap-2.5 animate-in fade-in zoom-in-95 duration-150">
            ...
            <form onSubmit={handleSubmit} className="space-y-4 text-xs">
              {/* Email Input */}
              <div>
                <label className="block font-semibold text-[var(--white-titanium)] mb-1.5 flex items-center justify-between">
                  ...
                  {email && !loading && (
                    <button
                      type="button"
                      onClick={() => setEmail('')}
                      className="text-[10px] text-[var(--silver-muted)] hover:text-[var(--white-titanium)] transition-colors"
                      title="Effacer / مسح"
                    >
                      <X className="w-3 h-3" />
                    </button>
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    placeholder="••••••••"
                    className="w-full bg-[var(--smoke)] border border-[var(--border-subtle)] focus:border-[#FF6B00] rounded-xl px-4 py-3 text-[var(--white-titanium)] font-mono outline-none transition-all pr-11 disabled:opacity-60 placeholder:text-[var(--silver-muted)]/60"
                  />
                  <button
                    type="button"
                    tabIndex={-1}
                    ...
                    className="absolute right-3 top-3 text-[var(--silver-muted)] hover:text-[var(--white-titanium)] transition-colors cursor-pointer p-0.5"
              ...
              <div className="pt-2">
                <p className="text-[10px] text-[var(--silver-muted)] text-center mb-1.5 font-mono">
                ...
                <div className="flex items-center justify-center gap-1.5 flex-wrap text-[10px]">
                  <button
                    ...
                    className="px-2.5 py-1 rounded-lg bg-[var(--smoke)] hover:bg-[var(--obsidian)] text-[#FF6B00] border border-[#FF6B00]/30 font-mono font-medium transition-colors cursor-pointer"
                  >
                    Admin
                  </button>
                  <button
                    ...
                    className="px-2.5 py-1 rounded-lg bg-[var(--smoke)] hover:bg-[var(--obsidian)] text-emerald-500 border border-emerald-500/30 font-mono font-medium transition-colors cursor-pointer"
                  >
                    Livreur
                  </button>
                  <button
                    ...
                    className="px-2.5 py-1 rounded-lg bg-[var(--smoke)] hover:bg-[var(--obsidian)] text-blue-500 border border-blue-500/30 font-mono font-medium transition-colors cursor-pointer"
                  >
                    Client
                  </button>
                </div>
              ...
              <div className="pt-2 text-center border-t border-[var(--border-subtle)]">
                <span className="text-[10px] font-mono text-[var(--silver-muted)] flex items-center justify-center gap-1">
...
export default function LoginPage() {
  return (
    <Suspense
      fallback={
        <div className="min-h-[85vh] flex items-center justify-center bg-[var(--obsidian)] text-[var(--silver-muted)]">
